...
The system SHALL provide a GET endpoint `/health` that returns the service status with all dependency checks.

#### Scenario: Healthy service
- **WHEN** a client sends a GET request to `/health`
- **THEN** the system returns HTTP 200 with a JSON body containing:
  - `status`: "ok"
  - `version` (string): Versión del microservicio
  - `timestamp` (ISO 8601): Timestamp de la respuesta
  - `dependencies` (object): Estado de cada dependencia
    - `database`: "ok" | "error"
    - `model`: "ok" | "error"
    - `chroma`: "ok" | "error"
